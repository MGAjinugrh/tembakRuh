...
pygame.display.set_caption("Tembak Ruh!")
...
def draw_bos_health_bar(surf, x, y, pct):
    pct = max(pct, 0)
    fill = (pct / 100) * BAR_LENGTH
    outline_rect = pygame.Rect(x, y, BAR_LENGTH, BAR_HEIGHT)
    fill_rect = pygame.Rect(x, y, fill, BAR_HEIGHT)
...
#     def __init__(self):
#             pygame.sprite.Sprite.__init__(self)
#             ## scale the boss img down
#             self.image = pygame.transform.scale(bos_img, (150, 100))
#             self.image.set_colorkey(BLACK)
#             self.rect = self.image.get_rect()
#             self.radius = 20
#             self.rect.centerx = WIDTH / 2
...
# ## defines the sprite for boss bullets
...
#         self.image.set_colorkey(BLACK)
#         self.rect = self.image.get_rect()
#         ## place the bullet according to the current position of the boss
#         self.rect.bottom = y 
#         self.rect.centerx = x
#         self.speedy = -10

#     def update(self):
#         """should spawn right in front of the boss"""
#         self.rect.y += self.speedy
...
# bos_img = pygame.image.load(path.join(img_dir, 'boss.png')).convert()
...
for i in range(9):
    ...
    # ## boss defeated
    # filename = 'boss_defeat0{}.png'.format(i)
    # img = pygame.image.load(path.join(img_dir, filename)).convert()
    # img.set_colorkey(BLACK)
...
expl_sounds = []
for sound in ['expl3.wav', 'expl3.wav', 'wikwik.wav', 'ouch.wav']:
    expl_sounds.append(pygame.mixer.Sound(path.join(sound_folder, sound)))
## main background music
#pygame.mixer.music.load(path.join(sound_folder, 'BackAlley Royale.ogg'))
...
while running:
    if menu_display:
        ...
        for i in range(manymobs):      ## 8 mobs when score is below 1000
            # mob_element = Mob()
            # all_sprites.add(mob_element)
            # mobs.add(mob_element)
            newmob()
    ...
    #if score > 70 :
    #    draw_bos_health_bar(screen, 5, 30, bos.health)
